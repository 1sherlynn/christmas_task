...
validate.extend(validate.validators.datetime, {
  ...
  format: function format(value, options) {
    if (options.dateOnly) return moment.utc(value).format('YYYY-MM-DD');
...
validate.extend(validate, {
   ...
   */
  middleware: function middleware(schema) {
    ...
    if (!_.isPlainObject(schema)) throw new Error('validation schema must be an object');

    if (_.isEmpty(schema)) throw new Error('validation schema cannot be empty/undefined');
    ...
    // was being skipped.
    if (!_(schema).omit(['path', 'query', 'body']).isEmpty()) throw new Error('validation schema had unsupported keys');
    ...
    return function (req, res, next) {
      var errors = _({}).merge(validate(req.params, schema.path)).merge(validate(req.query, schema.query)).merge(validate(req.body, schema.body)).value();
   ...
   */
  registerErrorFormatter: function registerErrorFormatter(formatFn) {
    if (formatFn !== undefined && !_.isFunction(formatFn)) throw new Error('error formatter must be a function');
   ...
   */
  registerValidator: function registerValidator(name, validatorFn) {
    var validator = {};
